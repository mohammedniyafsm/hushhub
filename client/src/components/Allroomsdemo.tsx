...
const RoomCard = ({ name, description, icon, color, time }: Item) => {
  return (
    <figure
      className={cn(
        "relative mx-auto w-full md:max-w-[700px] sm:max-w-[360px]  border cursor-pointer overflow-hidden rounded-2xl p-4",
    ...
    >
      <div className="flex items-start gap-3">
        {/* Icon */}
        <div
          className="flex items-center justify-center sm:w-10 sm:h-10 w-8 h-8 rounded-2xl text-xl shrink-0"
        ...
        <div className={`flex flex-col flex-1 gap-1 ${bricolage_grotesque}`}>
          <div className="flex items-center justify-between">
            <figcaption className="text-xs md:text-sm sm:text-lg font-semibold text-gray-900 dark:text-white">
              {name}
            </figcaption>
            <span className="text-xs text-gray-500">{time}</span>
          </div>
          <p className="md:text-sm  text-xs font-normal text-gray-600 dark:text-gray-300">
            {description}
          </p>
        </div>
      </div>
